...
Uses keymap `helm-buffer-map', which is not currently defined.

M-x helm-buffer-run-zgrep		Grep Buffer(s) works as zgrep too (C-u grep all buffers but non--file buffers).
M-x helm-buffers-run-multi-occur		Multi Occur buffer or marked buffers. (C-u toggle force searching current-buffer).
M-x helm-buffer-switch-other-window		Switch other window.
M-x helm-buffer-switch-other-frame		Switch other frame.
M-x helm-buffer-run-query-replace-regexp		Query replace regexp in marked buffers.
M-x helm-buffer-run-query-replace		Query replace in marked buffers.
M-x helm-buffer-run-ediff		Ediff current buffer with candidate.  If two marked buffers ediff those buffers.
M-x helm-buffer-run-ediff-merge		Ediff merge current buffer with candidate.  If two marked buffers ediff merge those buffers.
M-x helm-buffer-diff-persistent		Toggle Diff buffer with saved file without quitting.
M-x helm-buffer-revert-persistent		Revert buffer without quitting.
M-x helm-buffer-save-persistent		Save buffer without quitting.
M-x helm-buffer-run-kill-buffers		Delete marked buffers and quit.
M-x helm-buffer-run-kill-persistent		Delete buffer without quitting helm.
M-x helm-toggle-all-marks		Toggle all marks.
M-x helm-mark-all		Mark all.
M-x helm-toggle-buffers-details		Toggle details.
M-x helm-buffers-toggle-show-hidden-buffers		Show hidden buffers.
M-x helm-buffers-mark-similar-buffers		Mark all buffers with same type (color) than current.
...
Uses keymap `helm-find-files-map', which is not currently defined.

M-x helm-ff-run-locate		Run Locate (C-u to specify locate db, M-n insert basename of candidate)
M-x helm-ff-run-browse-project		Browse project (`C-u' recurse, `C-u C-u' recurse and refresh db)
M-x helm-ff-run-find-sh-command		Run Find shell command from this directory.
M-x helm-ff-run-grep		Run Grep (C-u Recursive).
M-x helm-ff-run-pdfgrep		Run Pdfgrep on marked files.
M-x helm-ff-run-zgrep		Run zgrep (C-u Recursive).
M-x helm-ff-run-grep-ag		Run AG grep on current directory.
M-x helm-ff-run-git-grep		Run git-grep on current directory.
M-x helm-ff-run-gid		Run gid (id-utils).
M-x helm-ff-run-etags		Run Etags (C-u use thing-at-point `C-u C-u' reload cache)
M-x helm-ff-run-rename-file		Rename File (C-u Follow).
M-x helm-ff-run-query-replace-on-marked		Query replace on marked files.
M-x helm-ff-run-copy-file		Copy File (C-u Follow).
M-x helm-ff-run-byte-compile-file		Byte Compile File (C-u Load).
M-x helm-ff-run-load-file		Load File.
M-x helm-ff-run-symlink-file		Symlink File.
M-x helm-ff-run-hardlink-file		Hardlink file.
M-x helm-ff-run-delete-file		Delete File.
M-x helm-ff-run-kill-buffer-persistent		Kill buffer candidate without quitting.
M-x helm-ff-persistent-delete		Delete file without quitting.
M-x helm-ff-run-switch-to-eshell		Switch to Eshell.
M-x helm-ff-run-eshell-command-on-file		Eshell command on file (C-u Apply on marked files, otherwise treat them sequentially).
M-x helm-ff-run-ediff-file		Ediff file.
M-x helm-ff-run-ediff-merge-file		Ediff merge file.
M-x helm-ff-run-complete-fn-at-point		Complete file name at point.
M-x helm-ff-run-switch-other-window		Switch other window.
M-x helm-ff-run-switch-other-frame		Switch other frame.
...
M-x helm-ff-rotate-left-persistent		Rotate Image Left.
M-x helm-ff-rotate-right-persistent		Rotate Image Right.
M-x helm-find-files-up-one-level		Go down precedent directory.
M-x helm-ff-run-switch-to-history		Switch to last visited directories history.
M-x helm-ff-file-name-history		Switch to file name history.
M-x helm-ff-properties-persistent		Show file properties in a tooltip.
M-x helm-mark-all		Mark all visibles candidates.
M-x helm-ff-run-toggle-auto-update		Toggle auto expansion of directories.
M-x helm-unmark-all		Unmark all candidates, visibles and invisibles.
M-x helm-ff-run-gnus-attach-files		Gnus attach files to message buffer.
M-x helm-ff-run-print-file		Print file, (C-u to refresh printers list).
M-x helm-enlarge-window		Enlarge helm window.
M-x helm-narrow-window		Narrow helm window.
M-x helm-ff-run-toggle-basename		Toggle basename/fullpath.
M-x helm-ff-run-find-file-as-root		Find file as root.
M-x helm-ff-run-find-alternate-file		Find alternate file.
...
M-x helm-find-files-up-one-level		Go down precedent directory.
M-x helm-ff-run-toggle-auto-update		Toggle auto expansion of directories.
M-x helm-ff-run-toggle-basename		Toggle basename.
M-x helm-ff-file-name-history		File name history.
C/M-x helm-cr-empty-string		Maybe return empty string (unless `must-match').
M-x helm-next-source		Goto next source.
M-x helm-previous-source		Goto previous source.
...
Uses keymap `helm-generic-files-map', which is not currently defined.

M-x helm-ff-run-toggle-basename		Toggle basename.
M-x helm-ff-run-grep		Run grep (C-u recurse).
...
M-x helm-ff-run-byte-compile-file		Byte compile file(s) (C-u load) (elisp).
M-x helm-ff-run-load-file		Load file(s) (elisp).
M-x helm-ff-run-ediff-file		Ediff file.
M-x helm-ff-run-ediff-merge-file		Ediff merge file.
M-x helm-ff-run-switch-other-window		Switch other window.
M-x helm-ff-properties-persistent		Show file properties.
M-x helm-ff-run-etags		Run etags (C-u use tap, C-u C-u reload DB).
...
Uses keymap `helm-grep-map', which is not currently defined.

M-x helm-goto-next-file		Next File.
M-x helm-goto-precedent-file		Precedent File.
M-x helm-yank-text-at-point		Yank Text at point in minibuffer.
M-x helm-grep-run-other-window-action		Jump other window.
M-x helm-grep-run-other-frame-action		Jump other frame.
M-x helm-grep-run-default-action		Run default action (Same as RET).
M-x helm-grep-run-save-buffer		Save to a `helm-grep-mode' enabled buffer.

* Helm PdfGrep Map

** Commands

Uses keymap `helm-pdfgrep-map', which is not currently defined.
...
Uses keymap `helm-etags-map', which is not currently defined.
...
M-x helm-goto-next-file		Next Buffer.
M-x helm-goto-precedent-file		Precedent Buffer.
M-x helm-yank-text-at-point		Yank Text at point in minibuffer.
M-x helm-moccur-run-goto-line-ow		Goto line in other window.
M-x helm-moccur-run-goto-line-of		Goto line in new frame.
